...
import { DetailRow, StarRating, icons } from "@/components/sections/shared";

export default function EVOverviewSections({ vehicle, mot_summary, tax_calculation, vehicle_stats, safety_rating }: {
          ...
          {vehicle_stats?.v5c_days_since !== null && vehicle_stats?.v5c_days_since !== undefined && vehicle_stats.v5c_days_since <= 90 && (
            <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mt-1 mb-1">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700">V5C issued {vehicle_stats.v5c_days_since} days ago &mdash; may indicate a recent ownership change.</p>
              </div>
            </div>
          )}
      ...
      {vehicle_stats && (
        <Card title="Vehicle Statistics" icon={icons.chart} status="neutral">
          ...
          {vehicle_stats.estimated_annual_mileage !== null && (
            <DetailRow label="Annual Average Mileage" value={`${vehicle_stats.estimated_annual_mileage.toLocaleString()} mi/yr`} />
          )}
          ...
          {vehicle_stats.mot_status_detail && (
            <DetailRow label="MOT" value={<Badge variant={vehicle_stats.mot_days_remaining !== null && vehicle_stats.mot_days_remaining < 0 ? "fail" : vehicle_stats.mot_days_remaining !== null && vehicle_stats.mot_days_remaining <= 30 ? "warn" : "pass"} label={vehicle_stats.mot_status_detail} />} />
          )}
          {vehicle_stats.tax_status_detail && (
            <DetailRow label="Tax" value={<Badge variant={vehicle_stats.tax_days_remaining !== null && vehicle_stats.tax_days_remaining < 0 ? "fail" : vehicle_stats.tax_days_remaining !== null && vehicle_stats.tax_days_remaining <= 30 ? "warn" : "pass"} label={vehicle_stats.tax_status_detail} />} />
          )}
            ...
            {[
              ...
              <div key={label}>
                ...
                <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${value >= 80 ? "bg-emerald-500" : value >= 60 ? "bg-amber-400" : "bg-red-400"}`}
                    style={{ width: `${value}%` }}
                  />
                </div>
              </div>
